...
/*************************************************************************************
    ...
    System.Multemic.Basedeck.KeyedDeck
...
namespace System.Multemic.Basedeck
{
                                                      ...
                                                      ICollection<IUnique<V>>, IProducerConsumerCollection<V>, IDisposable where V : IUnique
    {
        ...
        public SeededDeck(int capacity = 17, HashBits bits = HashBits.bit64) : base(bits)
        {
            size = capacity;
            minSize = capacity;
        ...
        public SeededDeck(IList<ICard<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity > collection.Count ? capacity : collection.Count, bits)
        {
            this.Add(collection);
        }
        public SeededDeck(IList<IUnique<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity > collection.Count ? capacity : collection.Count, bits)
        {
            foreach (var c in collection)
                this.Add(c);
        }
        public SeededDeck(IEnumerable<ICard<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity, bits)
        {
            this.Add(collection);
        }
        public SeededDeck(IEnumerable<IUnique<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity, bits)
        {
            foreach (var c in collection)
                this.Add(c);
        }
        ...
        protected ulong getPosition(long key, int tableSize)
        {
            // standard hashmap method to establish position / index in table 

             return ((ulong)key % ((ulong)(tableSize - 1)));
