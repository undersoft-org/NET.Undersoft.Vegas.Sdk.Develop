                ...
                if (Operand != OperandType.Like && Operand != OperandType.NotLike)
                {
                    switch (Operand)
                    {
                        case OperandType.Equal:
                            ...
                            r[fc.FilterRubric.FigureFieldId].ComparableInt64(fc.FilterRubric.RubricType).Equals(Value.ComparableInt64(fc.FilterRubric.RubricType)) :
                            r[fc.FilterRubric.FigureFieldId].ComparableDouble(fc.FilterRubric.RubricType).Equals(Value.ComparableDouble(fc.FilterRubric.RubricType)) : false);
                            break;
                        case OperandType.EqualOrMore:
                            ex = (r => r[fc.FilterRubric.FigureFieldId] != null ?
                             fc.FilterRubric.RubricType == typeof(IUnique) || fc.FilterRubric.RubricType == typeof(string) || fc.FilterRubric.RubricType == typeof(DateTime) ?
                              r[fc.FilterRubric.FigureFieldId].ComparableInt64(fc.FilterRubric.RubricType) >= (Value.ComparableInt64(fc.FilterRubric.RubricType)) :
                    ...
                    ex = (r => r[fc.FilterRubric.FigureFieldId] != null ? Convert.ChangeType(r[fc.FilterRubric.FigureFieldId], fc.FilterRubric.RubricType).ToString().Contains(Convert.ChangeType(Value, fc.FilterRubric.RubricType).ToString()) : false);
                else
                    ex = (r => r[fc.FilterRubric.FigureFieldId] != null ? !Convert.ChangeType(r[fc.FilterRubric.FigureFieldId], fc.FilterRubric.RubricType).ToString().Contains(Convert.ChangeType(Value, fc.FilterRubric.RubricType).ToString()): false);
            }
            return ex;
